...
export default function StudentsList({ students }: { students: Student[] }) {
  ...
  const filtered =
    filter === "all" ? students : students.filter((s) => (s.service_type ?? "assessoria") === filter);
        ...
        {FILTERS.map((f) => (
          <button
            ...
            className={`rounded-full px-4 py-1.5 text-sm font-medium transition-colors ${
              filter === f.value ? "bg-navy text-white" : "bg-lightblue/15 text-blue hover:bg-lightblue/25"
            }`}
          >
            {f.label}
          </button>
